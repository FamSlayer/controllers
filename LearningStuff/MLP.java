...
public class MLP implements FA<double[], double[]>, Evolvable
{
    ...
    public void mutate()
    {
        ...
        /* if(deviation<0.001){
        	System.out.println("Deviation too low man.");
        	deviation = 1.0f;
        }*/
    }
    ...
    private void mutate(double[] array)
    {
        for (int i = 0; i < array.length; i++)
        {
            ...
            array[i]+= random.nextGaussian()*deviation;
        }
    }
    ...
    public void psoRecombine(MLP last, MLP pBest, MLP gBest)
    {
        ...
        double num;
        //recombine using intermediate recombination assuming pBest and gBest are the two parents
        for(int i =0; i<inputs.length;i++){
        	for(int j=0;j<hiddenNeurons.length;j++){
        		num = random.nextGaussian();
        		if(num>0){
        			firstConnectionLayer[i][j] = (double) (pBest.firstConnectionLayer[i][j]);
        		}
        		else{
        			firstConnectionLayer[i][j] = (double) (gBest.firstConnectionLayer[i][j]);
        		}
        	}
        }
        ...
        for(int i=0;i<hiddenNeurons.length; i++){
        	for(int j=0;j<outputs.length;j++){
        		num = random.nextGaussian();
        		if(num>0){
        			secondConnectionLayer[i][j] = (double) (pBest.secondConnectionLayer[i][j]);
        		}
        		else{
        			secondConnectionLayer[i][j] = (double) (gBest.secondConnectionLayer[i][j]);
        		}
        	}
        }
